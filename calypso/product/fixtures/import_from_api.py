...
def save_image(url, product_code):
    r = requests.get(url)
    filename = getFilename_fromCd(r.headers.get('content-disposition')) or product_code
    path = f'/tmp/{filename}.jpg'
    open(path, 'wb').write(r.content)
    return path

def get_the_products():
    request = requests.get('https://api.lincocare.co.uk/product-categories/').json()
    for i in request:
        ...
        for product in i['products']:
            ...
            for image in product['images']:
                image_path = save_image(image['image_url'], product['product_code'])
